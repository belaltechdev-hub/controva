...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning(f"Redis not connected on startup: {e}")
    yield
    # --- Shutdown ---
...
async def client_dashboard(
    ...
    if client.usage_limit == 0:
        ...
        usage_percent = min((current_usage / client.usage_limit) * 100, 100)
    ...
    if usage:
        expiry_date = usage.start_date.replace(tzinfo=timezone.utc) + timedelta(days=client.validity_days)
    else:
        expiry_date = now + timedelta(days=client.validity_days)
...
async def api_check_usage(client_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # fallback to DB increment (only when Redis fails)
        usage.count += 1
        db.commit()
        current_usage = usage.count
...
async def api_reset(
    ...
    usage.count = 0
    usage.start_date = datetime.now(timezone.utc)
...
async def get_clients(db: Session = Depends(get_db), owner=Depends(require_owner)):
    ...
    for client in clients:
        ...
        if client.usage_limit == 0:
            ...
            percent = min((current_usage / client.usage_limit) * 100, 100)
